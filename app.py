...
def interpret_24hr_urine(urine_profile, patient_profile=None):
    ...
    if urine_profile["volume_L"] < 2.5:
        findings["urine_volume"] = (
            f"Low urine volume ({urine_profile['volume_L']} L/d). Goal is ~2.5 L/d for reducing recurrence risk."
        )

    # pH
    if urine_profile["ph"] < 6.0:
        findings["urine_ph"] = (
            f"Acidic urine pH ({urine_profile['ph']}). May increase risk of uric acid stones."
        )
    # Explicitly check for RTA
    elif patient_profile and "Renal Tubular Acidosis" in patient_profile.get(
        "medical_conditions", []
    ):
        # As per manuscript, pH >= 6.0 with RTA suggests CaP stones
        if urine_profile["ph"] >= 6.0:
            findings["urine_ph"] = (
                f"Alkaline urine pH ({urine_profile['ph']}) with diagnosed Renal Tubular Acidosis (RTA). Suggests a risk for calcium phosphate stones."
            )
    elif urine_profile["ph"] > 7.0:
        findings["urine_ph"] = (
            f"Very alkaline urine pH ({urine_profile['ph']}). May indicate urine infection by bacteria with urease and a risk for struvite stones."
        )

    # Calcium
    # Graded increase in risk from 150 mg/d as per manuscript
    if urine_profile["calcium_mg"] > 150:
        findings["urine_calcium"] = (
            f"Hypercalciuria ({urine_profile['calcium_mg']} mg/d). Levels >150 mg/d increase stone risk. Correlate with urine sodium."
        )

    # Oxalate
    if urine_profile["oxalate_mg"] > 40:
        findings["urine_oxalate"] = (
            f"Elevated urine oxalate ({urine_profile['oxalate_mg']} mg/d). Values >40 mg/d are excessive."
        )
        if urine_profile["oxalate_mg"] > 80:
            findings[
                "urine_oxalate"
            ] += " For values >80 mg/d, consider primary hyperoxaluria."

    # Citrate
    if urine_profile["citrate_mg"] < 400:
        findings["urine_citrate"] = (
            f"Low urine citrate ({urine_profile['citrate_mg']} mg/d). Values <400 mg/d may limit risk for calcareous stones."
        )

    # Uric Acid
    # Using ~750-800 mg/d as general upper limit
    if urine_profile["uric_acid_mg"] > 750:
        findings["urine_uric_acid"] = (
            f"High urine uric acid ({urine_profile['uric_acid_mg']} mg/d). Consider xanthine oxidase inhibitor or reduced purine intake if recurrent calcium oxalate or uric acid stones persist."
        )

    # Sodium
    # If hypercalciuria is present, sodium target is <100 mEq/d
    if "urine_calcium" in findings and urine_profile["sodium_mEq"] > 100:
        findings["urine_sodium"] = (
            f"High urine sodium ({urine_profile['sodium_mEq']} mEq). If hypercalciuria is present, a goal of <100 mEq/d is sought."
        )

    # Sulfate (indicative of animal protein intake)
    if urine_profile["sulfate_mmol"] > 30:
        findings["urine_sulfate"] = (
            f"High urine sulfate ({urine_profile['sulfate_mmol']} mmol/d). Suggests excessive dietary animal protein."
        )

    # Ammonium (indicative of acid production)
    if urine_profile["ammonium_mmol"] > 45:
        findings["urine_ammonium"] = (
            f"High urine ammonium ({urine_profile['ammonium_mmol']} mmol/d). Suggests excess acid production from diet, chronic diarrhea, or other cause."
        )

    # Cystine
    if urine_profile.get("cystine_mg", 0) > 30:  # Normal <30 mg/d
        findings["urine_cystine"] = (
            f"Elevated urine cystine ({urine_profile['cystine_mg']} mg/d). Normal individuals typically excrete <30 mg/d. Patients with cystinuria generally excrete >400 mg/d."
        )
    ...
    findings["supersaturation_targets"] = (
        "General supersaturation targets for reducing risk are <4 for calcium oxalate stones and <1 for calcium phosphate and uric acid stones."
    )
...
def generate_management_plan(
    stone_type, urine_interpretation, patient_profile, serum_labs=None
):
    """
    Generates a management plan based on stone type, urine interpretation, patient profile, and serum labs.
    """
    plan = [
        "Increase urine volume to ~2.5 L/day. This is always helpful in lowering supersaturation."
    ]
    ...
    if stone_type == "Calcium Oxalate":
        ...
        if (
            "urine_citrate" in urine_interpretation
            and "Low urine citrate" in urine_interpretation["urine_citrate"]
        ):
            plan.append(
                "Administer potassium citrate and/or treat potassium deficiency if hypocitraturic."
            )
        if (
            "urine_oxalate" in urine_interpretation
            and "Elevated urine oxalate" in urine_interpretation["urine_oxalate"]
        ):
            plan.append("Consider oxalate restriction for significant hyperoxaluria.")
        ...
        if (
            ...
            in patient_profile["medical_conditions"]
        ):
            plan.append(
        ...
        if (
            serum_labs
            and serum_labs.get("calcium_mg_dL", 0) >= 10.8
            and serum_labs.get("intact_pth_pg_mL", 0) >= 70
        ):
            plan.append(
        ...
        if (
            "urine_ph" in urine_interpretation
            and "Alkaline urine pH" in urine_interpretation["urine_ph"]
            and any(
                med in patient_profile.get("medications", [])
                for med in ["Topiramate", "Acetazolamide"]
            )
        ):
            plan.append(
        ...
        if "Renal Tubular Acidosis" in patient_profile["medical_conditions"] or (
            serum_labs and serum_labs.get("bicarbonate_mEq_L", 0) < 22
        ):
            plan.append(
        ...
        if (
            "urine_uric_acid" in urine_interpretation
            and "High urine uric acid" in urine_interpretation["urine_uric_acid"]
        ):
            plan.append(
        ...
        plan.append(
            "If stones persist despite initial measures, consider thiol drugs (tiopronin, penicillamine, captopril), acknowledging their cost and side effects."
        )
...
if page == "Patient Profile":
    ...
    with st.form("patient_profile_form", clear_on_submit=True):
        ...
        with col2:
            gender = st.selectbox(
                ...
                index=["Male", "Female", "Other"].index(
                    st.session_state["patient_profile"].get("gender", "Female")
                ),
            )
        with col3:
            num_prior_stones = st.selectbox(
                "Number of Prior Stone Episodes",
                options=list(range(0, 21)),
                index=max(
                    0,
                    min(
                        st.session_state["patient_profile"].get("num_prior_stones", 1),
                        20,
                    ),
                ),
            )

        first_stone_age = st.selectbox(
            "Age of First Stone Episode (if known)",
            options=list(range(0, 121)),
            index=max(
                0,
                min(
                    st.session_state["patient_profile"].get("first_stone_age", 28), 120
                ),
            ),
        )
        ...
        bmi = st.selectbox(
            "BMI (kg/m²)",
            options=[round(x * 0.1, 1) for x in range(100, 501)],
            index=max(
                0,
                min(
                    int(st.session_state["patient_profile"].get("bmi", 25.0) * 10), 5000
                ),
            ),
        )
        ...
        medical_conditions = st.multiselect(
            ...
            all_medical_conditions,
            default=st.session_state["patient_profile"].get("medical_conditions", []),
        )

        st.subheader("Current Medications (comma-separated if multiple)")
        medications_text = st.text_area(
            "List current medications (e.g., Topiramate, Hydrochlorothiazide)",
            value=", ".join(st.session_state["patient_profile"].get("medications", [])),
        )
        medications = [m.strip() for m in medications_text.split(",") if m.strip()]

        st.subheader("Dietary Habits")
        fluid_intake = st.number_input(
            "Daily Fluid Intake (L)",
            min_value=0.5,
            max_value=5.0,
            value=st.session_state["patient_profile"].get("fluid_intake_L", 2.5),
            step=0.1,
        )
        ...
        if submitted:
            st.session_state["patient_profile"] = {
                ...
                "first_stone_age": (
                    first_stone_age if first_stone_age > 0 else "unknown"
                ),
            ...
            st.write(
                "Debug: Patient profile updated in session state:",
                st.session_state["patient_profile"],
            )
        else:
...
# --- 24-Hour Urine Analysis Page ---
    ...
    with st.form("urine_analysis_form", clear_on_submit=True):
        ...
        with col_urine1:
            urine_profile_input["volume_L"] = st.selectbox(
                "Volume (L)",
                options=[round(x * 0.1, 1) for x in range(5, 26)],
                index=max(
                    0,
                    min(
                        round(
                            st.session_state["urine_profile"].get("volume_L", 2.5) * 10
                        )
                        - 5,
                        20,
                    ),
            ...
            urine_profile_input["ph"] = st.selectbox(
                "pH",
                options=[round(x * 0.1, 1) for x in range(0, 15)],
                index=max(
                    0,
                    min(
                        round(st.session_state["urine_profile"].get("ph", 6.0) * 10), 14
                    ),
                ),
            )
            urine_profile_input["calcium_mg"] = st.selectbox(
                "Calcium (mg)",
                options=list(range(0, 1001)),
                index=max(
                    0,
                    min(st.session_state["urine_profile"].get("calcium_mg", 200), 1000),
                ),
            )
            urine_profile_input["oxalate_mg"] = st.selectbox(
                "Oxalate (mg)",
                options=list(range(0, 101)),
                index=max(
                    0, min(st.session_state["urine_profile"].get("oxalate_mg", 35), 100)
                ),
            )

        with col_urine2:
            urine_profile_input["phosphorus_mg"] = st.selectbox(
                "Phosphorus (mg)",
                options=list(range(0, 1001)),
                index=max(
                    0,
                    min(
                        st.session_state["urine_profile"].get("phosphorus_mg", 800),
                        1000,
                    ),
                ),
            )
            urine_profile_input["uric_acid_mg"] = st.selectbox(
                "Uric Acid (mg)",
                options=list(range(0, 1001)),
                index=max(
                    0,
                    min(
                        st.session_state["urine_profile"].get("uric_acid_mg", 600), 1000
                    ),
                ),
            )
            urine_profile_input["sodium_mEq"] = st.selectbox(
                "Sodium (mEq)",
                options=list(range(0, 301)),
                index=max(
                    0,
                    min(st.session_state["urine_profile"].get("sodium_mEq", 150), 300),
                ),
            )
            urine_profile_input["potassium_mEq"] = st.selectbox(
                "Potassium (mEq)",
                options=list(range(0, 301)),
                index=max(
                    0,
                    min(
                        st.session_state["urine_profile"].get("potassium_mEq", 40), 300
                    ),
                ),
            )

        with col_urine3:
            urine_profile_input["magnesium_mg"] = st.selectbox(
                "Magnesium (mg)",
                options=list(range(0, 101)),
                index=max(
                    0,
                    min(st.session_state["urine_profile"].get("magnesium_mg", 60), 100),
                ),
            )
            urine_profile_input["sulfate_mmol"] = st.selectbox(
                "Sulfate (mmol)",
                options=list(range(0, 101)),
                index=max(
                    0,
                    min(st.session_state["urine_profile"].get("sulfate_mmol", 25), 100),
                ),
            )
            urine_profile_input["ammonium_mmol"] = st.selectbox(
                "Ammonium (mmol)",
                options=list(range(0, 101)),
                index=max(
                    0,
                    min(
                        st.session_state["urine_profile"].get("ammonium_mmol", 50), 100
                    ),
                ),
            )
            urine_profile_input["citrate_mg"] = st.selectbox(
                "Citrate (mg)",
                options=list(range(0, 1001)),
                index=max(
                    0,
                    min(st.session_state["urine_profile"].get("citrate_mg", 350), 1000),
                ),
            )
            urine_profile_input["cystine_mg"] = st.selectbox(
                "Cystine (mg, optional)",
                options=list(range(0, 101)),
                index=max(
                    0, min(st.session_state["urine_profile"].get("cystine_mg", 0), 100)
                ),
            )
        ...
        with col_serum1:
            serum_labs_input["calcium_mg_dL"] = st.selectbox(
                "Serum Calcium (mg/dL)",
                options=list(range(0, 101)),
                index=max(
                    0,
                    min(
                        int(st.session_state["serum_labs"].get("calcium_mg_dL", 9)), 100
                    ),
                ),
            )
            serum_labs_input["intact_pth_pg_mL"] = st.selectbox(
                "Intact PTH (pg/mL)",
                options=list(range(0, 301)),
                index=max(
                    0,
                    min(
                        int(st.session_state["serum_labs"].get("intact_pth_pg_mL", 30)),
                        300,
                    ),
                ),
            )
        with col_serum2:
            serum_labs_input["bicarbonate_mEq_L"] = st.selectbox(
                "Bicarbonate (mEq/L)",
                options=list(range(0, 101)),
                index=max(
                    0,
                    min(
                        int(
                            st.session_state["serum_labs"].get("bicarbonate_mEq_L", 24)
                        ),
                        100,
                    ),
                ),
            )
            serum_labs_input["potassium_mEq_L"] = st.selectbox(
                "Serum Potassium (mEq/L)",
                options=list(range(0, 101)),
                index=max(
                    0,
                    min(
                        int(st.session_state["serum_labs"].get("potassium_mEq_L", 4)),
                        100,
                    ),
                ),
            )
        with col_serum3:
            serum_labs_input["creatinine_mg_dL"] = st.selectbox(
                "Serum Creatinine (mg/dL)",
                options=list(range(0, 101)),
                index=max(
                    0,
                    min(
                        int(st.session_state["serum_labs"].get("creatinine_mg_dL", 0)),
                        100,
                    ),
                ),
            )
        ...
        if submitted_urine:
            ...
            if not st.session_state["patient_profile"]:
                ...
                data = {
                    "Category": ["Current Volume", "Goal Volume"],
...
# --- Acute Stone Management Page ---
    ...
    with st.form("acute_management_form"):
        ...
        hydronephrosis = st.checkbox("Hydronephrosis (on imaging)", value=False)
        aki = st.checkbox("Acute Kidney Injury", value=False)
        anuria = st.checkbox("Anuria", value=False)

        st.subheader("Known Stone Size")
        stone_size = st.radio(
            "Select stone size (if known)",
            ["< 5mm", "5-10mm", "> 10mm", "Unknown"],
            index=3,
        )
...
# --- Chronic Management Plan Page ---
    ...
    st.write(
        "Based on the patient's profile and urine analysis, here's the recommended long-term management."
    )
    ...
    if not st.session_state["patient_profile"] or not st.session_state["urine_profile"]:
        ...
        st.subheader("Confirmed Stone Type (If known, otherwise select most likely)")
        stone_type_options = [
            "Calcium Oxalate",
            "Calcium Phosphate",
            "Uric Acid",
        ...
        confirmed_stone_type = st.selectbox(
            "Select the primary stone type", stone_type_options, index=0
        )  # Default to Calcium Oxalate
        ...
        if st.button("Generate Chronic Plan"):
            ...
            st.info(
                "Long-term management requires periodic reevaluation of dietary changes and medications on urine indices as well as reassessments of stone burden by imaging (Figure 3)."
            )
...
# --- Educational Resources Page ---
    ...
    st.image(
        "figure1_pathogenesis.png",
        caption="Figure 1: Overview of Pathogenesis",
        use_container_width=True,
    )
    st.markdown("---")

    st.subheader("Common Stone Types")
    st.write("The type of stone formed significantly influences management strategies.")
    ...
    with st.expander("Calcium Oxalate Stones"):
        ...
        st.markdown("- **Increase urine volume** (target $\\sim2.5~L/d$).")
        st.markdown("- **Restrict sodium** ($<2,300~mg/d$).")
        st.markdown("- **Optimize calcium intake** ($1,000-1,200~mg/d$).")
        st.markdown("- Administer **thiazide** if hypercalciuric.")
        st.markdown("- Administer **potassium citrate** if hypocitraturic.")
        st.markdown("- **Oxalate restriction** for significant hyperoxaluria.")
        st.markdown("- **Restrict animal protein** and sucrose/fructose.")
    ...
    with st.expander("Calcium Phosphate Stones"):
        ...
        st.markdown("- **Increase urine volume** and **restrict sodium**.")
        st.markdown("- Administer **thiazide** if hypercalciuric.")
        st.markdown(
            "- Treat metabolic acidosis with **potassium citrate** while avoiding excessive urinary alkalinization."
        )
    ...
    with st.expander("Uric Acid Stones"):
        ...
        st.markdown("- Advise **lower animal protein intake**.")
        st.markdown(
            "- Consider **allopurinol** if hyperuricosuric and stones persist despite pH normalization."
        )
    ...
    with st.expander("Cystine Stones"):
        ...
        st.write("**Chronic Management Principles:**")
        st.markdown("- **Increase urine volume** to achieve urine cystine $<250~mg/L$.")
    ...
    st.markdown(
        "[Recurrence of Kidney Stone (ROKS) nomogram](https://www.qxmd.com/calculate/calculator_438/roks-recurrer-of-kidney-stone-2018)"
    )
    st.markdown(
        "[Kidney Stone Pathophysiology, Evaluation and Management: Core Curriculum 2023](https://www.ajkd.org/article/S0272-6386(23)00670-4/fulltext)"
    )
...
# --- Oxalate Content Finder Page ---
    ...
    if not oxalate_df.empty:
        ...
        if search_term:
            results = oxalate_df[
                oxalate_df["food"].str.contains(search_term, case=False)
                | oxalate_df["type"].str.contains(search_term, case=False)
            ]
            if not results.empty:
                st.write(f"Found {len(results)} matching items for '{search_term}':")
                st.dataframe(results)
            else:
                st.warning(f"No results found for '{search_term}'.")
        else:
            st.info("Enter a food item or category to search for its oxalate content.")
            st.write("Full Oxalate Database:")
            st.dataframe(oxalate_df)

    st.subheader("External Resources")
    st.markdown(
        "[University of Chicago Kidney Stone Center: How to Eat a Low Oxalate Diet](https://kidneystones.uchicago.edu/how-to-eat-a-low-oxalate-diet/)"
    )
...
st.sidebar.info(
    "Developed by a Nephrology Professor and Data Scientist for educational and clinical decision support based on the AJKD Core Curriculum 2023 on Kidney Stone Pathophysiology, Evaluation, and Management. This app is for informational purposes only and does not substitute professional medical advice."
)
